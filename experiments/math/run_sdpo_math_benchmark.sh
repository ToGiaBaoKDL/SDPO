...
RELIABILITY_GATE_START_THRESHOLD="${RELIABILITY_GATE_START_THRESHOLD:-0.2}"
...
TRAJECTORY_LOG_SAMPLES="${TRAJECTORY_LOG_SAMPLES:-16}"
TRAJECTORY_LOG_TEXT_CHARS="${TRAJECTORY_LOG_TEXT_CHARS:-6000}"
...
export TRAIN_MAX_SAMPLES VAL_MAX_SAMPLES SEED TRAJECTORY_LOG_SAMPLES TRAJECTORY_LOG_TEXT_CHARS TRAJECTORY_LOG_DIR
...
TRAJECTORY_LOG_DIR="${TRAJECTORY_LOG_DIR:-${LOG_DIR}/trajectories}"
export TRAJECTORY_LOG_DIR
mkdir -p "${LOG_DIR}"
...
echo "trajectory_log_dir=${TRAJECTORY_LOG_DIR} max_samples=${TRAJECTORY_LOG_SAMPLES} text_chars=${TRAJECTORY_LOG_TEXT_CHARS}"
...
run_sdpo_variant() {
  ...
  run_with_log "${exp_name}" \
    python3 -m verl.trainer.main_ppo \
      ...
      trainer.save_freq="${SAVE_FREQ}" \
      trainer.validation_data_dir="${LOG_DIR}/validation/${exp_name}" \
      trainer.trajectory_data_dir="${TRAJECTORY_LOG_DIR}" \
      trainer.trajectory_variant="${variant}" \
      trainer.trajectory_log_max_samples="${TRAJECTORY_LOG_SAMPLES}" \
      trainer.trajectory_log_text_max_chars="${TRAJECTORY_LOG_TEXT_CHARS}" \
      data.train_max_samples="${TRAIN_MAX_SAMPLES}" \
      data.val_max_samples="${VAL_MAX_SAMPLES}" \
