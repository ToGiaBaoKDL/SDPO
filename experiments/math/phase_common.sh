...
sdpo_math_configure_profile() {
  ...
  case "${hardware}:${profile}" in
    ...
    h200:fast)
      TRAIN_BS=80
      ROLLOUT_N=2
      AGENT_WORKERS="${AGENT_WORKERS:-20}"
      RESPONSE_LEN=1024
      MODEL_LEN=3072
      ACTOR_LEN=4096
      REPROMPT_LEN=2048
      ...
      SDPO_MAX_NUM_SEQS="${SDPO_MAX_NUM_SEQS:-128}"
      SDPO_GPU_UTIL="${SDPO_GPU_UTIL:-0.90}"
      SDPO_ACTOR_LEN="${SDPO_ACTOR_LEN:-4096}"
      SDPO_REPROMPT_LEN="${SDPO_REPROMPT_LEN:-2048}"
    ...
    h200:balanced)
      ...
      ACTOR_LEN=6144
      REPROMPT_LEN=3072
      BATCHED_TOKENS="${BATCHED_TOKENS:-262144}"
      MAX_NUM_SEQS="${MAX_NUM_SEQS:-160}"
      GPU_UTIL="${GPU_UTIL:-0.96}"
    ...
    h200:quality)
      ...
      ACTOR_LEN=8192
      REPROMPT_LEN=4096
      BATCHED_TOKENS="${BATCHED_TOKENS:-327680}"
      MAX_NUM_SEQS="${MAX_NUM_SEQS:-160}"
      GPU_UTIL="${GPU_UTIL:-0.96}"
      SDPO_BATCHED_TOKENS="${SDPO_BATCHED_TOKENS:-262144}"
      SDPO_MAX_NUM_SEQS="${SDPO_MAX_NUM_SEQS:-128}"
      SDPO_GPU_UTIL="${SDPO_GPU_UTIL:-0.88}"
      SDPO_ACTOR_LEN="${SDPO_ACTOR_LEN:-8192}"
      SDPO_REPROMPT_LEN="${SDPO_REPROMPT_LEN:-4096}"
      ENFORCE_EAGER="${ENFORCE_EAGER:-False}"
      ;;
